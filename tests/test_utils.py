...
KNOWN_RULES = {
    ...
    4: (np.array([-1.0, -1 / np.sqrt(5), 1 / np.sqrt(5), 1.0]),
        np.array([1 / 6, 5 / 6, 5 / 6, 1 / 6])),
    5: (np.array([-1.0, -np.sqrt(3 / 7), 0.0, np.sqrt(3 / 7), 1.0]),
        np.array([1 / 10, 49 / 90, 32 / 45, 49 / 90, 1 / 10])),
}


@pytest.mark.parametrize('n', sorted(KNOWN_RULES))
def test_known_rules(n):
...
def test_symmetry_and_weight_sum(n):
    xk, wk = quadrature_lobatto(n=n)
...
def test_polynomial_exactness(n):
    ...
    for deg in range(2 * n - 2):
        ...
        np.testing.assert_allclose(np.sum(wk * xk**deg), exact,
                                   rtol=1e-8, atol=1e-8,
                                   err_msg=f"degree {deg}")


def test_rescaled_interval():
    a, b = 0.0, np.pi
    n = 50
...
def test_returned_arrays_are_writable_and_independent():
...
def test_integrate_with_provided_nodes():
    theta = np.linspace(0.0, np.pi, 1801)
    f = np.sin(theta)
    xk, wk = quadrature_lobatto(abscissa_min=0.0, abscissa_max=np.pi, n=len(theta))
    res_provided = integrate_lobatto(f, theta, xk=xk, wk=wk, assume_sorted=True)
    res_default = integrate_lobatto(f, theta, assume_sorted=True)
    np.testing.assert_allclose(res_provided, res_default, rtol=1e-12)


def test_integrate_unsorted_input():
...
def test_integrate_lp_parameter():
    theta = np.linspace(0.0, np.pi, 501)
    f = np.sin(theta)
